...
// --label_map_file=.../labelmap_inspur.prototxt
// root_dir = /home/hh/v2i/data/image/2_persecond/
// list_file = /home/ai/ssd/caffe/data/Inspur/trainval.txt
// out_dir = /home/data/Inspur/lmdb/Inspur_trainval_lmdb
...
int main(int argc, char** argv) {
  ...
  std::ifstream infile(argv[2]); // listfile, i.e. img_path label_path              // 定义infile，来自/home/ai/ssd/caffe/data/Inspur/trainval.txt
  ...
  if (anno_type == "classification") {
    ...
    while (infile >> filename >> labelname) {
      lines.push_back(std::make_pair(filename, labelname));                         // lines:= (filename, labelname)
    }
  }
  if (FLAGS_shuffle) {
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
    ...
    // filename = root_folder + lines[line_id].first;                                  // 拼接出图像的绝对路径  ==> filename
    ...
    if (anno_type == "classification") {
      ...
      // labelname = root_folder + boost::get<std::string>(lines[line_id].second);     // 拼接出图像标注文件的绝对路径 ==> labelname
      labelname = boost::get<std::string>(lines[line_id].second);
      LOG(INFO) << "read annotation file: " << labelname;
      status = ReadRichImageToAnnotatedDatum(filename, labelname, resize_height,
          resize_width, min_dim, max_dim, is_color, enc, type, label_type,
          name_to_label, &anno_datum);                                              // 图像和标注等信息写入anno_datum
    ...
    // Put in db
    string out;
    CHECK(anno_datum.SerializeToString(&out));                                      // 序列化 anno_datum 到 out 流
    txn->Put(key_str, out);                                                         // 把 key 和 out 流写入数据库缓存区

    if (++count % 1000 == 0) {
      // Commit db
      txn->Commit();                                                                // 把缓存区的1000个记录一次性写入数据库
